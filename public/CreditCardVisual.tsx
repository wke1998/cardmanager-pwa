...
export const CreditCardVisual: React.FC<Props> = ({ card, onClick }) => {
  ...
  return (
    <div
      onClick={onClick}
      className={`relative w-full max-w-lg mx-auto aspect-[1.586/1] rounded-[20px] p-4 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.5)] overflow-hidden cursor-pointer active:scale-[0.97] transition-all duration-300 border border-white/10 ${card.backgroundImage ? 'bg-gray-900' : `bg-gradient-to-br ${gradient}`}`}
    >
      {/* 處理自訂圖片背景 */}
      ...
      <div className="flex flex-col h-full justify-between relative z-10 text-white">
        ...
        <div className="flex justify-between items-center mb-1">
          <p className="text-sm sm:text-lg font-medium tracking-wider text-white/90 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] truncate max-w-[60%]">{card.bankName}</p>

          <div className="flex flex-col items-end">
            <span className="text-[8px] sm:text-[9px] tracking-widest text-white/70 uppercase drop-shadow-md">本期累積</span>
            <span className="font-mono text-xs sm:text-base font-bold leading-none drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] text-white/95">
        ...
        <div className="flex flex-col mt-auto mb-2">
          <div className="w-8 h-5.5 bg-gradient-to-br from-yellow-200 via-yellow-400 to-yellow-600 rounded-[3px] border border-yellow-700/50 flex items-center justify-center mb-1 shadow-md relative overflow-hidden opacity-90">
            <div className="absolute inset-0 border-[0.5px] border-black/20 rounded-[3px]"></div>
            ...
            <div className="w-4.5 h-3 border-[0.5px] border-black/30 rounded-sm bg-gradient-to-br from-yellow-300 to-yellow-500 z-10"></div>
          </div>

          <h3 className="text-xl sm:text-3xl font-bold tracking-widest drop-shadow-[0_2px_6px_rgba(0,0,0,0.8)] truncate text-white/95">
            {card.name}
          </h3>
        </div>
        ...
        <div className="flex justify-between items-end">
          <div className="flex flex-col gap-1 sm:flex-row sm:gap-4">
            <div className="flex items-baseline gap-1.5">
              <span className="text-[8px] sm:text-[9px] uppercase tracking-widest text-white/70 drop-shadow-md">結帳</span>
              <span className="font-mono text-sm sm:text-lg font-medium leading-none drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">{card.statementDate} <span className="text-[9px] font-sans text-white/60">日</span></span>
            </div>
            <div className="flex items-baseline gap-1.5">
              <span className="text-[8px] sm:text-[9px] uppercase tracking-widest text-white/70 drop-shadow-md">繳款</span>
              <span className="font-mono text-sm sm:text-lg font-medium leading-none drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">{card.dueDate} <span className="text-[9px] font-sans text-white/60">日</span></span>
            </div>
          </div>
